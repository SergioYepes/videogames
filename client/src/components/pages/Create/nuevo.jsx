...
function Nueva() {
  ...
  function handleSubmit(e){
    ...
    if(!Object.getOwnPropertyNames(error).length && Object.values(Formulario).every(Boolean)){
  ...
  return (
    <div className="divCreate">
      ...
      <form className="form" onSubmit={e=>handleSubmit(e)}>
        <h1 className='tittleC'>New Game creation </h1>
        <br/>
          <div>
          ...
          <div>
            <h4><label>Description: </label></h4>
              ...
              />
              {error.description && (
                        <p className='error'><strong>{error.description}</strong></p>
                    )}
          </div>
          <div>
            <h4><label>Relased: </label></h4>
              <input 
              type="date" 
              name="released"
              ...
              />
              {error.released && (
                        <p className='error'><strong>{error.released}</strong></p>
                    )}
          </div>
          <div>
            <h4><label>Platforms: </label></h4>
              <input 
              type="text" 
              name="platforms"
              ...
              onChange={e=>handleChange(e)}
              />
              {error.platforms && (<p className='error'><strong>{error.platforms}</strong></p>)}
              
          </div>
          <div>
            <h4><label>Rating: </label></h4>
              <input 
              type="text" 
              name="rating"
              ...
              onChange={e=>handleChange(e)}
              />
              {error.rating && (<p className='error'><strong>{error.rating}</strong></p>)}
          </div>
          <div>
          ...
          <div>
            ...
            Formulario.genres.map(e=>{
                return (
                  <ul key={e} className='allGenres'>
                    <li>
                      <p className='genres'><strong>{e}</strong></p>
            ...
            {error.genres && (<p className='error'><strong>{error.genres}</strong></p>)}
          </div>
          <button type="submit" className='boop' ><strong>Punch</strong></button>
